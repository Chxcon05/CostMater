...
export async function authRoutes(app) {
  app.post('/api/auth/register', registerLimiter, async (req, res) => {
    ...
    try {
      const existing = await get('SELECT id FROM users WHERE email = ?', [normalizeEmail(email)]);
      ...
      await run('INSERT INTO users (name, email, password_hash, role) VALUES (?, ?, ?, ?)', [name.trim(), normalizeEmail(email), passwordHash, role]);

      const user = await get('SELECT id, name, email, role FROM users WHERE email = ?', [normalizeEmail(email)]);

      if (!user) {
        throw new Error('Usuario no encontrado después de crear');
      }

      await audit(user.id, 'users', user.id, 'REGISTER', null, publicUser(user));
  ...
  app.post('/api/auth/login', loginLimiter, async (req, res) => {
    ...
    try {
      const user = await get('SELECT * FROM users WHERE email = ?', [normalizeEmail(email)]);
      ...
      const token = generateToken(user);
      await audit(user.id, 'users', user.id, 'LOGIN', null, null);
  ...
  app.post('/api/auth/logout', authenticateToken, async (req, res) => {
    try {
      await audit(req.user.id, 'users', req.user.id, 'LOGOUT', null, null);
  ...
  app.post('/api/auth/forgot-password', async (req, res) => {
    ...
    try {
      const user = await get('SELECT id, email FROM users WHERE email = ?', [normalizeEmail(email)]);
      if (!user) {
      ...
      await run('INSERT INTO password_resets (user_id, token, expires_at) VALUES (?, ?, ?)', [user.id, token, expiresAt]);

      await audit(user.id, 'users', user.id, 'PASSWORD_RESET_REQUESTED', null, null);
  ...
  app.post('/api/auth/reset-password', async (req, res) => {
    ...
    try {
      ...
      const passwordHash = await bcrypt.hash(password.trim(), 10);
      await run('UPDATE users SET password_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [passwordHash, reset.user_id]);
      await run('UPDATE password_resets SET used = 1 WHERE id = ?', [reset.id]);

      await audit(reset.user_id, 'users', reset.user_id, 'PASSWORD_RESET', null, null);
  ...
  app.put('/api/auth/change-password', authenticateToken, async (req, res) => {
    ...
    try {
      ...
      await run('UPDATE users SET password_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [passwordHash, req.user.id]);

      await audit(req.user.id, 'users', req.user.id, 'PASSWORD_CHANGED', null, null);
      res.json({ success: true, message: 'Contraseña actualizada correctamente' });
    } catch (error) {
  ...
  app.get('/api/auth/audit', authenticateToken, async (req, res) => {
    try {
      const logs = await all('SELECT * FROM audit_log WHERE user_id = ? ORDER BY created_at DESC LIMIT 50', [req.user.id]);
      res.json(logs);
    } catch (error) {
